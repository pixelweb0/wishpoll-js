    ...
    return (
        <div className="contents">
					...
					<ul>
						...
						<li>
							...
							<div className="chk-radio-box">
								<span><input type="radio" id="chk-radio1" name="sch-type" /><label htmlFor="chk-radio1" onClick={e => setType(e.target.innerText)}>A/B테스트 모음</label></span>
								<span><input type="radio" id="chk-radio2" name="sch-type" /><label htmlFor="chk-radio2" onClick={e => setType(e.target.innerText)}>간단 설문</label></span>
								<span><input type="radio" id="chk-radio3" name="sch-type" /><label htmlFor="chk-radio3" onClick={e => setType(e.target.innerText)}>스티커 퀘스트</label></span>
								<span><input type="radio" id="chk-radio4" name="sch-type" /><label htmlFor="chk-radio4" onClick={e => setType(e.target.innerText)}>미디어 업로드</label></span>
								<span><input type="radio" id="chk-radio5" name="sch-type" /><label htmlFor="chk-radio5" onClick={e => setType(e.target.innerText)}>데이터 태그</label></span>
								<span><input type="radio" id="chk-radio6" name="sch-type" /><label htmlFor="chk-radio6" onClick={e => setType(e.target.innerText)}>음성 퀘스트</label></span>
								<span><input type="radio" id="chk-radio7" name="sch-type" /><label htmlFor="chk-radio7" onClick={e => setType(e.target.innerText)}>GPS</label></span>
								<span><input type="radio" id="chk-radio8" name="sch-type" /><label htmlFor="chk-radio8" onClick={e => setType(e.target.innerText)}>QR코드</label></span>
								<span><input type="radio" id="chk-radio9" name="sch-type" /><label htmlFor="chk-radio9" onClick={e => setType(e.target.innerText)}>참여신청</label></span>
							</div>
						</li>
					</ul>
				</div>
				...
				<table>
					<thead>
						<tr>
							...
							<th scope="col">퀘스트 제목</th>
							<th scope="col">퀘스트 유형</th>
							<th scope="col">기간</th>
							<th scope="col">리워드</th>
					...
					<tbody>
						{polllist.map((qdata, Q_Idx)=>(
							<tr key={Q_Idx} name={Q_Idx} onClick={() => {
								...
								<td>{qdata.Title}</td>
								<td className="type">{qdata.Type === 0 ? "A/B테스트 모음" : qdata.Type === 1 ? "간단 설문" : qdata.Type === 2 ? "스티커 퀘스트" : qdata.Type === 3 ? "미디어 업로드" : qdata.Type === 4 && "데이터 태그"}</td>
			...
			<section id="poll1_btn-wrap">
				<Link to='poll-gamelist2' className="btn btnCF">폴 등록</Link>
				<a href="#" className="btn btnCF">퀘스트 등록</a>
			</section>
		</div>
    )
...
export default Poll1TotalList2;